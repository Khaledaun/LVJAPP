...
## 2026-04-23 · Sprint 0.7 — Playwright EN + AR visual regression baseline (infra)

Branch: `claude/sprint-0.7-playwright-baseline-7fKjM`.

Closes the *infrastructure* side of EXECUTION_PLAN §10.4 exit
criterion ("Visual regression baseline recorded for EN + AR"). The
baseline **PNGs** are generated by a second, trivial PR because
Playwright screenshots are platform-specific and cannot be authored
outside the CI runner that runs the comparison; this PR lands the
spec, config, scripts, and doc pointers so the follow-up is a
PNG-commit plus a `test.skip` flip.

**Code.**

- `e2e-tests/visual-regression.spec.ts` — new. Full-page screenshots
  of `/` (landing) and `/signin` (the two-panel design with the
  Arabic motto) across both locales (`en`, `ar`) via the existing
  `lvj_locale` cookie contract. Gated on `RUN_VISUAL_REGRESSION=1`
  so CI doesn't red-flag the spec before baselines exist.
  Deterministic setup: frozen `Date.now()` init script,
  `emulateMedia({ reducedMotion: 'reduce' })`, animations-disabled
  style injection, `document.fonts.ready` wait for
  `next/font/google`.
- `playwright.config.ts` — new `visual-regression` project pinned
  to chromium with a deterministic `1280 × 800` viewport and a
  `testMatch` scoped to the visual spec. The other five projects
  (chromium/firefox/webkit/Mobile Chrome/Mobile Safari) gain a
  matching `testIgnore` so the visual spec runs exactly once per
  test per locale (cross-browser diff noise is too high to be a
  useful gate).
- `package.json` — new scripts `test:e2e:visual` (run the
  comparison) and `test:e2e:visual:update` (regenerate baselines
  via `--update-snapshots`). Both set `RUN_VISUAL_REGRESSION=1` so
  callers don't have to think about the env gate.

**Docs.**

- `docs/EXECUTION_PLAN.md` — bumped version header from v1.1 to
  v1.2 (A-010-R2 gate). §10.1 current position refreshed: PR #13
  merged, PR #14 open, this branch is 0.7-baseline-infra, next in
  D-019 order after baselines land is Sprint 8.5. §10.4 gains a
  new *Exit status (2026-04-23)* sub-block that names the two-step
  close-out (baseline-commit PR → blocking-gate PR) and warns that
  until step 2 lands, the manual gate is `RUN_VISUAL_REGRESSION=1
  npm run test:e2e:visual`. Deliverable (6) added for the new
  visual-regression spec.
- `docs/EXECUTION_LOG.md` — this entry.

**No CI changes.** The visual-regression project is intentionally
*not* in the `smoke` npm script or `legacy-checks` job yet —
baselines must land first. Once they do, a trivial follow-up PR
drops the `test.skip` in the spec, adds `playwright test
--project=visual-regression` to `smoke` (or a new `smoke:visual`),
and Sprint 0.7 closes formally.

**No D-NNN.** Single-browser visual regression + `RUN_VISUAL_
REGRESSION` env gate + two-step baseline close-out are all
implementation details of §10.4, not architectural decisions. If
CI wiring changes in the follow-up (e.g. we split visual
regression into its own GitHub Actions job rather than folding it
into `legacy-checks`), that's still implementation — no decision.

**Exit criteria (this PR).**

- `e2e-tests/visual-regression.spec.ts` present + idle (gated).
- `playwright.config.ts` has a `visual-regression` project + the
  other projects ignore the spec.
- `package.json` has the two new scripts.
- EXECUTION_PLAN §10.4 carries an explicit exit-status note so
  future sessions know Sprint 0.7 is 80% closed, not closed.

**Deferred (to the Sprint-0.7-baseline follow-up PR).**

- Generate baseline PNGs in CI, commit under
  `e2e-tests/visual-regression.spec.ts-snapshots/`.
- Drop the `test.skip(!RUN_VR, ...)` gate in the spec.
- Add the visual-regression project to the `smoke` script and to
  the `legacy-checks` job (blocking once stable; informational for
  the first week of runs to catch antialiasing drift).
...
- [ ] Sprint-0.7-baseline follow-up PR — generate visual-regression baseline PNGs on CI (`RUN_VISUAL_REGRESSION=1 npm run test:e2e:visual:update`), commit under `e2e-tests/visual-regression.spec.ts-snapshots/`, drop the `test.skip` gate in the spec, add the `visual-regression` project to `smoke` or a new `smoke:visual`, and fold it into `legacy-checks` (informational for the first week, then blocking). Closes Sprint 0.7 formally.
